...
import type {
  ...
  SharedItemByType,
  TempItemType,
  TAudioNodeInletConnection,
  TAudioNodeOutletConnection,
} from "../../../types";
...
import {
  IInletsMeta,
  IOutletsMeta,
} from "../../../objects/base/AbstractObject";

export const isJSPatcherObjectConstructor = (
  x: any,
): x is typeof AbstractObject =>
  typeof x === "function" && x?.isJSPatcherObjectConstructor;

export const isJSPatcherObject = (x: any): x is AbstractObject =>
  typeof x === "object" && x?.isJSPatcherObject;

export type THardwareMetaType =
  | "anything"
  | "signal"
  | "object"
  | "number"
...
export type IPropsMeta<T extends Record<string, any> = Record<string, any>> = {
  [K in keyof T]: IPropMeta<T[K]>;
};
export interface IHardwarePatcherObjectMeta<
...
export type Data<T> = T extends IHardwarePatcherObject<
  infer D,
  any,
  any,
  any,
...
>
  ? D
  : never;
export type State<T> = T extends IHardwarePatcherObject<
  any,
  infer S,
  any,
  any,
...
>
  ? S
  : never;
export type Ios<T> = T extends IHardwarePatcherObject<
  any,
  any,
  infer IO,
  any,
...
>
  ? IO
  : never;
export type PatcherInlets<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  infer I,
...
>
  ? I
  : never;
export type PatcherOutlets<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  any,
...
>
  ? O
  : never;
export type Args<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  any,
...
>
  ? A
  : never;
export type Props<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  any,
...
>
  ? P
  : never;
export type UIState<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  any,
...
>
  ? U
  : never;
export type EventMap<T> = T extends IHardwarePatcherObject<
  any,
  any,
  any,
  any,
...
>
  ? E
  : never;
export type TIoEvent<
  IO extends any[] = any[],
  $ extends keyof Pick<IO, number> = keyof Pick<IO, number>,
> = { io: $; data: IO[$] };
export type HardwarePatcherObjectEventMap<
  D,
  S,
  IO extends any[],
  I extends any[],
...
> = {
  ...
  metaUpdated: {
    oldMeta: IHardwarePatcherObjectMeta;
    meta: IHardwarePatcherObjectMeta;
  };
...
export interface IHardwarePatcherObject<
  ...
  U extends {} = {},
  E extends Partial<
    HardwarePatcherObjectEventMap<D, S, IO, I, O, A, P, U, {}>
  > &
    Record<string, any> = {},
> extends TypedEventEmitter<
    HardwarePatcherObjectEventMap<D, S, IO, I, O, A, P, U, E>
  > {
  /** Should be true */
   ...
   */
  getSharedItem<T extends ProjectItemType>(
    id?: string,
    type?: T,
    data?: () => Promise<TempItemByType<T>["data"]>,
    onceCreate?: (aitem: SharedItemByType<T>) => any,
  ): Promise<{
    id: string;
    item: SharedItemByType<T>;
    newItem: boolean;
    off?: () => any;
  }>;
  // output to console
...
export interface AnyJSPatcherObject
  extends IHardwarePatcherObject<
    Record<string, any>,
    Record<string, any>,
    any[],
    any[],
...
export default abstract class AbstractObject<
  ...
  extends TypedEventEmitter<
    HardwarePatcherObjectEventMap<D, S, IO, I, O, A, P, U, E>
  >
  implements IHardwarePatcherObject<D, S, IO, I, O, A, P, U, E>
{
  ...
  private _meta = (this.constructor as typeof AbstractObject)
    .meta as IHardwarePatcherObjectMeta<P>;
  ...
  getProp<K extends keyof P = keyof P>(key: K): P[K] {
    ...
    return typeof this.box.props[key] === "undefined"
      ? this.meta.props[key].default
      : this.box.props[key];
  }
  ...
  async updateArgs(args: A, options?: ObjectUpdateOptions) {
    if (args?.length) {
      ...
      if (options?.undoable)
        this.undoable({ oldArgs, args: this.args.slice() as A });
    }
  }
  ...
  async updateState(state: Partial<S>, options?: ObjectUpdateOptions) {
    if (state && Object.keys(state).length) {
      ...
      if (options?.undoable)
        this.undoable({ oldState, state: { ...this.state } });
    }
  }
  fn<$ extends keyof Pick<IO, number> = keyof Pick<IO, number>>(
    io: $,
    data: IO[$],
  ) {
    if (io === 0) {
      ...
      if (data !== null && typeof data === "object") {
        ...
        if (
          propsInKeys.length &&
          propsInKeys.every((k) => propsKeys.indexOf(k) !== -1)
        ) {
  ...
  outlet<$ extends keyof Pick<IO, number>>(io: $, data: IO[$]) {
    if (io >= this.ios.length) return;
    Array.from(this.ioLines[io])
      .sort(Line.compare)
      .map((line) => line.pass(data));
  }
  ...
  async getSharedItem<T extends ProjectItemType>(
    id = this.box.id,
    type: T = "unknown" as T,
    data?: () => Promise<TempItemByType<T>["data"]>,
    onceCreate?: (aitem: SharedItemByType<T>) => any,
        ...
        if (data) {
          ...
          try {
            item = (await tempMgr.root.addFile(
              id,
              d,
              type as TempItemType,
            )) as SharedItemByType<T>;
            newItem = true;
          } catch {
            item = tempMgr.getProjectItemFromPath(id) as SharedItemByType<T>;
          }
