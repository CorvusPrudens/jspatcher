...
export default class GlobalPackageManager {
  ...
  get builtInPackagesNames() {
    return [
      "Base",
      ...this.importedPackages.filter((p) => p.isBuiltIn).map((p) => p.name),
    ];
  }
  ...
  async init() {
    ...
    this.jsaw = {
      Base: await (
        await import("./objects/base/index.jsdsppkg.main")
      ).default(),
    };
    this.daisy = {
      Base: await (
        await import("./hardware/objects/base/index.hwpkg")
      ).default(),
    };
    // this.gen = {
  ...
  add(pkgIn: TPackage, lib: PatcherMode, pathIn: string[] = []) {
    ...
    while (path.length) {
      ...
      if (!pkg[key]) pkg[key] = {};
      else if (isJSPatcherObjectConstructor(pkg[key]))
        pkg[key] = { [ImporterDirSelfObject]: pkg[key] } as any;
  ...
  async getModuleFromURL(url: string, id?: string) {
    ...
    if (!Object.keys(m).length)
      throw new Error(`Module ${id} from ${url} is empty`);
  ...
  async resolveModule(m: any, url: string, id?: string, isBuiltIn = false) {
    if (typeof m === "object" && m !== null) {
      if (m.default?.isJSPatcherPackage) {
        ...
        const p: IExternalPackage = {
          ...m.default,
          baseUrl: new URL(".", new URL(url, location.href)).href,
          isBuiltIn,
        };
        if (p.jspatpkg) {
          const url = new URL(p.jspatpkg, p.baseUrl).href;
          const fetched = await this.fetchModule(url);
          const getter: PackageGetter =
            typeof fetched === "function" ? fetched : fetched.default;
        ...
        if (p.hwpkg) {
          const url = new URL(p.hwpkg, p.baseUrl).href;
          const fetched = await this.fetchModule(url);
          const getter: PackageGetter =
            typeof fetched === "function" ? fetched : fetched.default;
          this.add(await getter(this.env), "daisy", [p.name]);
        }
