...
export default class Patcher extends FileInstance<
  PatcherEventMap,
  PersistentProjectFile | TempPatcherFile
> {
  ...
  static async fromProjectItem(options: {
    file: PersistentProjectFile | TempPatcherFile;
    env: IJSPatcherEnv;
    project?: IProject;
    instanceId?: string;
  }): Promise<Patcher> {
  ...
  constructor(options: {
    ...
    this.props = {
      ...
      lineIndexCount: 0,
      objectInit:
        typeof options.objectInit === "boolean" ? options.objectInit : true,
    };
  }
  get state() {
    return this._state;
  ...
  async init(data = this.file?.data, fileName = this.fileName) {
    if (data instanceof ArrayBuffer) {
      ...
      const extMap: Record<string, PatcherMode> = {
        bell: "bell",
        maxpat: "max",
        gendsp: "gen",
        dsppat: "faust",
  ...
  async load(patcherIn: RawPatcher | TMaxPatcher | any, modeIn?: PatcherMode) {
    ...
    await this.env.taskMgr.newTask(
      this,
      "Loading patcher...",
      async (onUpdate) => {
        ...
        if (patcher.props)
          this.props = { ...this.props, ...patcher.props, mode };
        ...
        this.emit("ready");
        await Promise.all(
          Object.keys(this.boxes).map((id) => this.boxes[id].postInit()),
        );
  ...
  async unload() {
    ...
    if (Object.keys(this.boxes).length) {
      this._state.preventEmitChanged = true;
      await Promise.all(
        Object.keys(this.boxes).map((id) => this.boxes[id].destroy()),
      );
      this._state.preventEmitChanged = false;
      this.emitGraphChanged();
    }
  ...
  getLinesByBox(
    ...
    srcOuts.forEach((idOut) =>
      destIns.forEach((idIn) =>
        idIn === idOut ? result.push(idIn) : undefined,
      ),
    );
    return result;
  }
    ...
    for (const boxId in this.boxes) {
      const box = this.boxes[boxId] as Box<
        IJSPatcherObject<
          ...
          any[],
          any[],
          { description: string; type: TMetaType }
        >
      >;
  ...
  get publicProps() {
    const { dependencies, bgColor, editingBgColor, grid, openInPresentation } =
      this.props;
    return {
      dependencies,
      bgColor,
