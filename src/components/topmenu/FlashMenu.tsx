...
export default class FlashMenu extends React.PureComponent<P, S> {
  ...
  doDownload = async (firmware: ArrayBuffer) => {
    if (this.state.device) {
      ...
      const { TransferSize } = this.state.device.properties || {
        TransferSize: 1024,
      };
  ...
  handleClickDownload = async () => {
    ...
    try {
      ...
      const filename =
        this.props.env.activeEditor.instance.file?.name
          .replace("/", "_")
          .replace(".bell", ".zip") || "project.zip";

      webSocket.onopen = (event) => {
        try {
          this.setState({
            build_message: "Building program...",
            building: true,
            build_error: false,
            progress: null,
          });
          ...
          this.setState({
            build_message: "Unexpected error occurred. Please try again.",
            building: false,
            build_error: true,
          });
        }
      ...
      if (typeof error === "string") {
        ...
        if (key === "InternalError") {
          ...
          if (inner_key === "CompileError") {
            this.props.env.newLog(
              "none",
              inner_key,
              error[key][inner_key].program.makefile,
            );
            this.props.env.newLog(
              "none",
              inner_key,
              error[key][inner_key].program.cpp,
            );
            this.props.env.newLog(
              "none",
              inner_key,
              error[key][inner_key].stderr,
            );
          } else {
            this.props.env.newLog(
              "none",
              inner_key,
              JSON.stringify(error[key][inner_key]),
            );
          }
        } else {
  ...
  handleClickBuild = async () => {
    ...
    try {
        ...
        try {
          ...
          if (data) {
            ...
            this.props.env.newLog(
              "none",
              "USB",
              `Flashing ${buffer.byteLength} bytes to device.`,
            );
      ...
      webSocket.onerror = () => {
        this.setState({
          build_message: "Error connecting to server. Please try again.",
          building: false,
          build_error: true,
        });
      };
  ...
  connect = async (deviceIn: dfu.Device) => {
    ...
    if (desc && Object.keys(desc).length > 0) {
      ...
      if (
        ...
        deviceIn.settings.alternate.interfaceProtocol == 0x02
      ) {
  ...
  autoConnect = async (vid: number, pid: number) => {
    ...
    for (let dfu_device of dfu_devices) {
      if (
        dfu_device.device_.vendorId == vid &&
        dfu_device.device_.productId == pid
      ) {
  ...
  render() {
    ...
    return (
      <div className="loader-container">
          ...
          <Dropdown item={true} icon={false} text="Daisy">
            <Dropdown.Menu style={{ minWidth: "max-content" }}>
              ...
              <Dropdown.Item
                ...
                text="Program"
                description={`${ctrlKey} + Shift + F`}
                disabled={this.state.locked || this.state.building}
              />
              <Dropdown.Item
                onClick={this.handleClickDownload}
                text="Download Hardware Project"
                disabled={this.state.locked || this.state.building}
              />
        ...
        {this.state.building ? (
          <div className="monitor">{this.state.build_message}</div>
        ) : (
          <div></div>
        )}
        {this.state.build_error ? (
          <div className="monitor"> {this.state.error_message} </div>
        ) : (
          <div></div>
        )}
      </div>
