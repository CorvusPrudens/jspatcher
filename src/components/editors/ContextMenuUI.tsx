                ...
                {!this.props.editor.isLocked ? (
                  <>
                    <Menu.Item
                      ...
                      onClick={() => {
                        this.setState({ open: false });
                        this.props.editor.setState({ locked: true });
                      }}
                    />
                    <Divider />
                  </>
                  ...
                  <Menu.Item
                    ...
                    onClick={() => {
                      this.setState({ open: false });
                      this.props.editor.setState({ locked: false });
                    }}
                  />
                )}
              </>
              ...
              <>
                {this.state.singleSelection ? (
                  <>
                    <Menu.Item
                      key="help"
                      content={`Open ${this.state.singleSelection} Help`}
                      icon="help"
                      onClick={this.handleOpenHelp}
                      disabled={this.state.help.length === 0}
                    />
                    <Menu.Item
                      key="reference"
                      content={`Open ${this.state.singleSelection} Reference`}
                      icon="book"
                      onClick={this.handleOpenReference}
                      disabled={!Boolean(this.state.reference)}
                    />
                    <Menu.Item
                      key="dock"
                      content="Open in dock"
                      icon="edit"
                      onClick={this.handleContextDockable}
                      disabled={!this.state.dockable}
                    />
                    <Divider />
                  </>
                ) : undefined}
                ...
                <Menu.Item
                  ...
                  disabled={this.state.clipboardEmpty}
                />
                <Menu.Item
                  key="delete"
                  content="Delete"
                  ...
                  onClick={() => {
                    this.setState({ open: false });
                    this.props.editor.deleteSelected();
                  }}
                  disabled={this.state.selection.length == 0}
                />
                {this.state.selection.length == 0 ? undefined : (
                  <>
                    ...
                    <Menu.Item
                      ...
                      onClick={this.handleSendToBack}
                    />
                    {this.props.editor instanceof PatcherEditor ? (
                      <>
                        <Menu.Item
                          key="presadd"
                          content="Add to presentation"
                          icon="video camera"
                          onClick={this.handleAddToPres}
                          disabled={!this.state.enableAddPres}
                        />
                        <Menu.Item
                          key="presremove"
                          content="Remove from presentation"
                          icon="video camera"
                          onClick={this.handleRemoveFromPres}
                          disabled={!this.state.enableRemovePres}
                        />
                      </>
                    ) : undefined}
                  </>
