...
export interface IPackageManager extends TypedEventEmitter<PackageManagerEventMap> {
  ...
  searchInPkg(
    query: string,
    limit: number,
    staticMethodOnly: boolean,
    pkg: TPackage,
    path?: string[]
  ): { path: string[]; object?: typeof IJSPatcherObject | TPackage }[];
  searchInLib(
    query: string,
    limit: number,
    staticMethodOnly: boolean,
    lib: TFlatPackage
  ): { key: string; object: typeof IJSPatcherObject }[];
  getFromPath(pathIn: (string | symbol)[], pkg: TPackage): TPackage | typeof IJSPatcherObject;
}

export default class PackageManager
  extends TypedEventEmitter<PackageManagerEventMap>
  implements IPackageManager
{
  ...
  get packagesInfo() {
    return Object.keys(this.global[this.mode]).map(
      (id) =>
        ({
          id,
  ...
  async init() {
    ...
    this.pkg = {};
    for (const pkgName of Object.keys(this.global[this.mode]).sort((a, b) =>
      b === "globalThis" ? -1 : 1
    )) {
  ...
  async loadPatcherDependencies() {
    try {
      await this.patcher.env.taskMgr.newTask(
        this,
        `${this.patcher.file?.name || ""} Loading dependencies`,
        async (onUpdate: (newMsg: string) => any) => {
  ...
  searchInPkg(
    query: string,
    limit = Infinity,
    staticMethodOnly = false,
    pkg: TPackage,
    path: string[] = []
  ): { path: string[]; object?: typeof IJSPatcherObject | TPackage }[] {
    const outs: {
      path: string[];
      object?: typeof IJSPatcherObject | TPackage;
    }[] = [];
