...
export type PrefixKeys<I, P extends string> = {
  [K in keyof I & string as `${P}${Capitalize<K>}`]: I[K];
};

export type PatcherMode = "max" | "gen" | "faust" | "bell" | "jsaw" | "daisy";
...
export type FileExtension =
  | PatcherFileExtension
  | AudioFileExtension
  | TextFileExtension
  | ImageFileExtension
  | VideoFileExtension
  | HardwareFileExtension;
...
export type SharedItemByType<T extends ProjectItemType> = T extends "patcher"
  ? TempPatcherFile | PersistentProjectFile
  : T extends "audio"
  ? TempAudioFile | PersistentProjectFile
  : T extends "text"
  ? TempTextFile | PersistentProjectFile
  : T extends "image"
  ? PersistentProjectFile
  : T extends "video"
  ? PersistentProjectFile
  : T extends "hardware"
  ? TempHardwareFile | PersistentProjectFile
  : TempData;

export type TempItemByType<T extends ProjectItemType> = T extends "patcher"
  ? TempPatcherFile
  : T extends "audio"
  ? TempAudioFile
  : T extends "text"
...
export type ProjectItemDataType<T extends ProjectItemType = any> =
  T extends "folder"
    ? RawProjectItems
    : T extends "patcher"
    ? RawPatcher
...
export type TAbstractPackage = {
  [key: string]: ObjectDescriptor | TAbstractPackage;
};
...
export type TAudioNodeInletConnection<T = AudioNode | AudioParam> = {
  node: T;
  index?: T extends AudioNode ? number : never;
};
...
export type TResizeHandlerType =
  | "n"
  | "ne"
  | "e"
  | "se"
