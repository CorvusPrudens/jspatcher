...
export default class EditorContainer extends TypedEventEmitter<EditorContainerEventMap> {
  ...
  get isDirty(): boolean {
    return (
      !this.editors.every((i) => !i.isDirty) &&
      this.children.every((c) => !c.isDirty)
    );
  }
  ...
  handleOpenEditor = (editor: EnvEventMap["openEditor"]) => {
    ...
    if (
      editor instanceof PatcherEditor ||
      editor instanceof HardwarePatcherEditor
    ) {
      const onHelp = async () => {
        const boxes = editor.state.selected
          .filter((id) => id.startsWith("box") && editor.boxes[id])
          .map((id) => editor.boxes[id]);
        ...
        if (box.meta.helpFiles.length > 0) {
          ...
          try {
            ...
            const patcher = new Patcher({
              env: this._env,
              project: this._env.currentProject,
            });
            ...
            this._env.newLog(
              "error",
              "Help File",
              "Error fetching help file. Please try again.",
            );
          }
        } else {
          this._env.newLog(
            "info",
            "Help File",
            "This object does not yet have a help file.",
          );
        }
      };
  ...
  split(direction: TSplitDirection) {
    if (this.mode !== "none")
      throw new Error(`The Container is already splitted as ${this.mode}`);
    ...
    if (direction === "bottom" || direction === "right")
      this.children = [container, newContainer];
