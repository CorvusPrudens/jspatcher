...
class ObjectsItems extends React.PureComponent<
  {
...
> {
  ...
  get list() {
    ...
    if (search) {
      const searchResult = this.props.editor.instance.state.pkgMgr.searchInPkg(
        search,
        128,
        false,
        pkg,
      ...
      searchResult.forEach((r, i) => {
        ...
        const sel =
          ...
          selected.every((e, i) => e === path[i]);
        if (typeof object === "object") {
          list.push(
            <List.Item
              key={i}
              icon={sel ? "folder open" : "folder"}
              className={key ? "folder-item" : "abstract folder-item"}
              header={path.join(".")}
          ...
          const props = sel
            ? { className: "selected", description: this.getDescription(path) }
            : {};
          list.push(
            <List.Item
              key={i}
    ...
    if (pathIn.length && ImporterDirSelfObject in pkg) {
      ...
      const props = sel
        ? {
            className: "abstract selected",
            description: this.getDescription(path),
          }
        : { className: "abstract" };
      list.push(
        <List.Item
          key={"__JSPatcher_ImporterDirSelfObject"}
          {...props}
          icon="window maximize"
          header={pathIn[pathIn.length - 1]}
          onClick={() => this.handleSelect(path)}
          onMouseDown={(e: React.MouseEvent) => this.handleMouseDown(e, path)}
        />,
      );
    }
  ...
  handleMouseDown = (e: React.MouseEvent, path: (string | symbol)[]) => {
    ...
    // Find Patcher Div
    const patcherDiv = document.body.querySelector(
      `.patcher-container[data-id="${this.props.editor.editorId}"]`,
    );
...
export default class Objects extends React.PureComponent<
  { env: Env },
  { editor: PatcherEditor | HardwareEditor; pkg: TPackage; search: string }
> {
  state = {
    editor:
      this.props.env.activeEditor instanceof PatcherEditor || this.props.env.activeEditor instanceof HardwareEditor
        ? this.props.env.activeEditor
        : null,
    pkg:
      this.props.env.activeEditor instanceof PatcherEditor || this.props.env.activeEditor instanceof HardwareEditor
        ? this.props.env.activeEditor.instance.activePkg
        : null,
    search: "",
  };
  timer: number = undefined;
  handlePkgChanged = ({ pkg }: { pkg: TPackage }) =>
    this.setState({ pkg: {} }, () => this.setState({ pkg }));
  handleEnvActiveEditor = ({ editor }: EnvEventMap["activeEditor"]) => {
    this.state.editor?.instance.off("libChanged", this.handlePkgChanged);
    if (editor instanceof PatcherEditor || editor instanceof HardwareEditor) {
  ...
  componentDidMount() {
    if (this.state.editor instanceof PatcherEditor)
      ...
      <>
        ...
        <Menu icon inverted size="mini">
          <Input
            ...
            icon={
              this.state.search
                ? { name: "close", link: true, onClick: this.handleClickClose }
                : "search"
            }
            placeholder="Search..."
