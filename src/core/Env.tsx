...
import {
  ...
  initializeWamEnv,
  initializeWamGroup,
} from "@webaudiomodules/sdk";
import TypedEventEmitter, {
  ITypedEventEmitter,
} from "../utils/TypedEventEmitter";
import GlobalPackageManager from "./GlobalPackageManager";
import PersistentProjectItemManager, {
  IPersistentProjectItemManager,
} from "./file/PersistentProjectItemManager";
...
export interface EnvEventMap {
  ...
  activeEditorContainer: {
    editorContainer: EditorContainer;
    oldEditorContainer: EditorContainer;
  };
...
export default class Env
  extends TypedEventEmitter<EnvEventMap>
  implements IJSPatcherEnv
{
  readonly thread = "main";
  readonly generatedId = new Uint32Array(
  ...
  readonly fileMgr: PersistentProjectItemManager =
    new PersistentProjectItemManager(this, this.fileMgrWorker);
  readonly tempMgr: TemporaryProjectItemManager =
    new TemporaryProjectItemManager(this);
  ...
  async init() {
    ...
    if (!this.options.noUI && this.divRoot)
      ReactDOM.render(<UI env={this} />, this.divRoot);

    await this.taskMgr.newTask(
      this,
      "Initializing Bell Environment...",
      async () => {
        ...
        await this.taskMgr.newTask(
          ...
          async (onUpdate) => {
            ...
            if (projectZip) {
              ...
              if (this.fileMgr.projectRoot.existItem(filename))
                await this.fileMgr.projectRoot.findItem(filename).destroy();
        ...
        await this.taskMgr.newTask(
          ...
          async (onUpdate) => {
            const packagesRes = await fetch(
              "./packages/internal-packages.json",
            );
            const packages = await packagesRes.json();
            for (const p of packages) {
              onUpdate(p);
              await this.pkgMgr.importFromURL(
                `./packages/${p}/index.js`,
                undefined,
                true,
              );
            ...
            const hardwarePackageRes = await fetch(
              "./packages/internal-packages-hardware.json",
            );
            const hardwarePackages = await hardwarePackageRes.json();
            for (const p of hardwarePackages) {
              onUpdate(p);
              await this.pkgMgr.importFromURL(
                `./packages/${p}/index.js`,
                undefined,
    ...
    this.audioCtx.destination.channelCount =
      this.audioCtx.destination.maxChannelCount;
    this.loaded = true;
    await this.emit("ready");
    const { file, server, room } = urlParamsOptions;
    if (server)
      await this.liveShare.login(server, this.options.liveShare.nickname);
    if (room)
      await this.liveShare.join(room, "", this.options.liveShare.nickname);
    if (file) {
      try {
  ...
  generateId(objectIn: object) {
    return (
      ...
      (globalThis.Atomics?.add(this.generatedId, 0, 1) ?? ++this.generatedId[0])
    );
  }
  ...
  set divRoot(root: HTMLDivElement) {
    ...
    if (!this.options.noUI && this._divRoot)
      ReactDOM.unmountComponentAtNode(this._divRoot);
    this._divRoot = root;
