...
export default class HardwareUI extends React.PureComponent<P, S> {
  ...
  handleReady = () => {
    ...
    this.setState({
      bgColor: editor.props.bgColor,
      editingBgColor: editor.props.editingBgColor,
      presentation: editor.state.presentation,
    });
  ...
  handleLockedChange = (locked: boolean) =>
    this.setState({ locked: this.props.runtime || locked });
  handlePresentationChange = (presentation: boolean) =>
  ...
  handleEditingBgColorChange = (editingBgColor: string) =>
    this.setState({ editingBgColor });
  handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (
      div.scrollWidth !== this.size.width ||
      div.scrollHeight !== this.size.height
    ) {
      shouldUpdate = true;
  ...
  handleMouseDown = (e: React.MouseEvent) => {
    ...
    const handleDraggable = () => {
      ...
      if (
        patcherRect.top + patcherDiv.clientHeight < e.clientY ||
        patcherRect.left + patcherDiv.clientWidth < e.clientX
      )
        return; // Click on scrollbar
      let patcherPrevScroll = {
        left: patcherDiv.scrollLeft,
        top: patcherDiv.scrollTop,
      };
      const selectedBefore = this.props.editor.state.selected.slice();
      const selectionRect = [
        e.clientX - patcherRect.left + patcherDiv.scrollLeft,
        e.clientY - patcherRect.top + patcherDiv.scrollTop,
        0,
        0,
      ] as TRect;
      const handleMouseMove = (e: MouseEvent) => {
        ...
        if (x > patcherRect.width - 10)
          patcherDiv.scrollLeft += x + 10 - patcherRect.width;
        if (y < 10) patcherDiv.scrollTop += y - 10;
        if (y > patcherRect.height - 10)
          patcherDiv.scrollTop += y + 10 - patcherRect.height;
      };
  ...
  handleClick = (e: React.MouseEvent) => {
    if (this.props.runtime) return;
    const ctrlKey =
      this.props.editor.env.os === "MacOS" ? e.metaKey : e.ctrlKey;
    if (ctrlKey && !this.props.editor.state.selected.length)
  ...
  handleDoubleClick = (e: React.MouseEvent) => {
    ...
    if (
      !(
        ...
        (e.target.classList.contains("boxes") ||
          e.target.classList.contains("patcher"))
      )
    )
      return;
    ...
    const x = round(
      Math.max(0, e.clientX - patcherRect.left + patcherDiv.scrollLeft),
      gridX,
    );
    const y = round(
      Math.max(0, e.clientY - patcherRect.top + patcherDiv.scrollTop),
      gridY,
    );
    const { presentation } = patcher.state;
    this.props.editor.createBox({
      text: "",
      ios: [],
      rect: [x, y, 0, 0],
      presentation,
  ...
  handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (
      ...
      (e.key === "n" || e.key === "s" || e.key === "k" || e.key === "c") &&
      !e.ctrlKey &&
      !e.shiftKey &&
      !e.altKey &&
      !e.metaKey
      ...
      const x = round(
        Math.max(
          0,
          this.cachedMousePos.x - patcherRect.left + patcherDiv.scrollLeft,
        ),
        gridX,
      );
      const y = round(
        Math.max(
          0,
          this.cachedMousePos.y - patcherRect.top + patcherDiv.scrollTop,
        ),
        gridY,
      );
      let text = "";
      ...
      else if (mode === "daisy" && e.key === "c") text = "comment";
  ...
  handleMouseMove = (e: React.MouseEvent) =>
    (this.cachedMousePos = { x: e.clientX, y: e.clientY });
  componentDidMount() {
    const { editor } = this.props;
  ...
  render() {
    ...
    let selectionDiv;
    if (
      selectionRect[2] !== selectionRect[0] ||
      selectionRect[3] !== selectionRect[1]
    ) {
    ...
    const backgroundColor = this.props.transparent
      ? "transparent"
      : this.state.locked
      ? this.state.bgColor
      : this.state.editingBgColor;
    return (
      <div
        ref={this.refDiv}
        ...
        onScroll={
          this.handleScroll
        } /*  onDragEnter={this.handleDragEnter} onDragOver={this.handleDragOver} onDragLeave={this.handleDragLeave} onDrop={this.handleDrop} */
      >
    ...
    Object.keys(created.lines).forEach((id) => {
      ...
      this.lines[line.id] = (
        <LineUI
          {...this.props}
          id={line.id}
          key={this.state.timestamp + line.id}
        />
      );
    });
  ...
  render() {
    return (
      <div className="boxes" style={this.state}>
        {Object.entries(this.boxes)
          .sort(([a], [b]) => this.zIndexes[a] - this.zIndexes[b])
          .map(([, jsx]) => jsx)}
      </div>
    );
  }
...
class Grid extends React.PureComponent<GridProps, GridState> {
  ...
  handleGridChange = (grid: [number, number]) =>
    this.setState({ grid: grid.slice() as [number, number] });
  handleEditingBgColorChange = (editingBgColor: string) =>
    this.setState({ editingBgColor });
  componentDidMount() {
    const { editor } = this.props;
  ...
  render() {
    ...
    const style = {
      backgroundImage: `${sBGImageX}, ${sBGImageY}`,
      backgroundSize: `${pxx} ${pxy}`,
      width,
      height,
    };
    return <div className="grid-background" style={style} />;
  }
}
